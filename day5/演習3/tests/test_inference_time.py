...
def test_model_inference_time(model_path, test_data):
    ...
    # 推論結果の検証
    assert len(predictions) == len(test_data), "予測結果のサンプル数が入力と一致しません"
    assert all(isinstance(pred, (int, np.int64)) or (isinstance(pred, float) and pred.is_integer()) 
              for pred in predictions), "予測結果は整数値である必要があります"
    
    # 推論時間の検証（100サンプルで0.5秒以内を期待）
    assert inference_time < 0.5, f"推論時間が許容範囲を超えています: {inference_time:.4f}秒"
